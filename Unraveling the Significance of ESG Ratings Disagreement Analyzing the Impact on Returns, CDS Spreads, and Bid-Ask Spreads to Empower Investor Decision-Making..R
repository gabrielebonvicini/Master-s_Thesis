...
##Assess if winsorizing is needed
...
##Assess the correct length of the y-axis 
min(total_percentage)
max(total_percentage)

##Scatter plot of returns and transaction costs 
plot(tcost_percentage*100, x = date_axes, xaxt = "n" , yaxt = "n" ,  
     ylim = c(-10,10), cex = 0.3, pch = 20, 
     xlab =  "Dates", ylab = "percentage" ) #costs 
points(y = total_percentage*100, x = date_axes, 
       col = "red", cex = 0.3, pch = 20) #returns 
legend("topright", legend = c("Costs", "Returns"), col = c("black", "red"),
...
plot(x = date_axes, y = cumu_sum_ret, type = "l", xaxt = "n", yaxt = "n" ,  
     ylim = c(-50,50), ylab = "percentage", xlab = "Dates", col = "red") #rets
axis.Date(1, at = seq(date_axes[1], date_axes[length(date_axes)] , "6 months") , 
          labels = T, format = "%m/%Y", tcl = -0.5) #add an axes with dates 
legend("topright", legend = c( "Cumulative returns"), 
       col = c("red"), lwd = 2)
